<template>
  <router-link :to="tool.path" class="block group">
    <Card
      class="tool-card relative overflow-hidden transition-[shadow,transform,border-color] duration-250 hover:shadow-lg hover:-translate-y-1 border-border/50 hover:border-primary/30 min-h-[160px]"
      :data-tool="tool.id"
    >
      <!-- Left accent bar -->
      <div
        class="absolute top-4 bottom-4 left-0 w-1.5 rounded-r transition-[width] duration-250 group-hover:w-2"
        :class="toolAccentClass"
      />
      
      <!-- Bottom glow -->
      <div
        class="absolute bottom-0 left-0 right-0 h-0.5 opacity-40 transition-[opacity,height] duration-250 group-hover:opacity-70 group-hover:h-1"
        :class="toolGlowClass"
      />
      ...
      <CardContent class="flex items-center gap-5 p-8 pl-10">
        ...
        >
          ...
          <span
            v-else
            class="text-3xl transition-[color] duration-250"
            :class="toolIconClass"
          >
            {{ tool.icon }}
          </span>
